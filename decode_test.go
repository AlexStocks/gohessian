...
func TestDecodeNull(t *testing.T) {
	...
	// h := NewDecoder(bytes.NewReader(data.Bytes()))
	h := NewDecoder(append(REPLY, 'N'))
	v, err := h.Decode()
	if err != nil {
...
func TestDecodeInt32(t *testing.T) {
	...
	// h := NewDecoder(bytes.NewReader(data.Bytes()))
	h := NewDecoder(append(REPLY, []byte{'I', 0, 0, 0, 89}...))
	v, err := h.Decode()
	if err != nil {
		t.Fatalf("error: %v", err)
	}
...
func TestDecodeLong(t *testing.T) {
	...
	// h := NewDecoder(bytes.NewReader(data.Bytes()))
	h := NewDecoder(append(REPLY, []byte{'L', 0, 0, 0, 0, 1, 47, 129, 172}...))
	v, err := h.Decode()
	if err != nil {
		t.Fatalf("error: %v", err)
	}
...
func TestDecodeDate(t *testing.T) {
	...
	// h := NewDecoder(bytes.NewReader(data.Bytes()))
	h := NewDecoder(append(REPLY, []byte{'d', 0, 0, 1, 67, 206, 0, 226, 40}...))
	v, err := h.Decode()
	if err != nil {
		t.Fatalf("error: %v", err)
	}
...
func TestDecodeDouble(t *testing.T) {
	...
	// h := NewDecoder(bytes.NewReader(data.Bytes()))
	h := NewDecoder(append(REPLY, []byte{'D', 64, 159, 20, 61, 217, 127, 98, 183}...))
	v, err := h.Decode()
	if err != nil {
		t.Fatalf("error: %v", err)
	}
...
func TestDecodeListWithoutType(t *testing.T) {
	b := []byte{
...
func TestDecodeListWithType(t *testing.T) {
	b := []byte{
...
func (f Foo) GetType() string {
	return "github.com/AlexStocks/codec/hessian/Foo"
}
...
func (f *Foo) SetBar(bar int64) { // !!!注意此处：之所以使用int64，是因为encode.go中把int按照int64处理了
	f.bar = int(bar)
}

func (f *Foo) GetBaz() string {
	return f.baz
}

func (f *Foo) SetBaz(baz string) {
	f.baz = baz
}
